...
struct Rect {
    ...
    Rect(int x_, int y_, int w_, int h_) noexcept : x(x_), y(y_), width(w_), height(h_) {}

    // Derived properties
    int Left() const noexcept { return x; }
    int Top() const noexcept { return y; }
    int Right() const noexcept { return x + width; }    // exclusive if width is size
    int Bottom() const noexcept { return y + height; }  // exclusive if height is size
